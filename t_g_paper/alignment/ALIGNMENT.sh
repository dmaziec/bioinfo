...
known_sites_indel="/n/data1/hms/dbmi/park/DATA/MosaicForecastResources/interval_sets/af-only-gnomad.raw.sites.b37.vcf.gz" #"/n/data1/hms/dbmi/park/dominika/testing/tg_paper/resources/Mills_and_1000G_gold_standard.indels.hg19.sites.vcf.gz"
...
ln -s ${reference_genome}.fai reference.fasta.fai
ln -s ${reference_genome}.dict reference.dict
...
echo "Running Sentieon Realigner"
if [ "$force_rerun" = "true" ] || ! [ -f realigned.bam ]; then
/n/data1/hms/dbmi/park/dominika/testing/tg_paper/scripts/sentieon_Realigner.sh deduped.bam $reference_genome $known_sites_indel || \
  { echo "Error in Sentieon Realigner" && rm -f realigned.bam && exit 1; }
fi
