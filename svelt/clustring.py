...
def create_reports(missing_samples_df, histology):
    ...
    if number_of_samples_missing > 0:
        ...
        report_file.write(
            f"Number of samples available in SVELT but not available in the publication: {number_of_samples_missing}\n"
        )
    ...
    for his in histology_abb:
        ...
        if len(publication_data_subset) > 2:
            ...
            scipy_dendrogram = scipy.cluster.hierarchy.dendrogram(
                cluster_map.dendrogram_row.linkage, labels=publication_data_subset.index
            )
